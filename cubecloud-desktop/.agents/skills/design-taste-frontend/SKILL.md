...
description: Anti-slop renderer design rules for cubecloud-desktop surfaces (Memory tabs, Plans, Skills, Settings, modals, dialogs). Adapted from upstream taste-skill v2  -  brief inference, three dials (VARIANCE / MOTION / DENSITY), AI-tell bans, strict pre-flight check.
source: community
metadata:
...
# Frontend Taste for cubecloud-desktop (anti-slop, v2)
...
- `DESIGN_VARIANCE` (1-10): 1 = perfect symmetry, 10 = artsy chaos.
  *Default for cubecloud-desktop: 4* (app chrome is information-dense, not
  art).
- `MOTION_INTENSITY` (1-10): 1 = static, 10 = cinematic.
  *Default for cubecloud-desktop: 2* (subtle: hover states, modal fades,
  accordion expand). Anything above 4 needs `prefers-reduced-motion`
  honoring.
- `VISUAL_DENSITY` (1-10): 1 = art gallery, 10 = cockpit.
  *Default for cubecloud-desktop: 6* (it's a power-user app; users want
...
Modals are an *interruption*, not a page. The cubecloud-desktop
...
- No `bg-purple-500` accent by default. cubecloud-desktop uses a single
...
in a hero. For cubecloud-desktop:
...
templated. cubecloud-desktop's settings and plans surfaces have
...
- It is not a design system. cubecloud-desktop does not have a design
...
- [ ] **Dial values** explicit (VARIANCE / MOTION / DENSITY), not
  silently using the cubecloud-desktop defaults?
...
- v2 (this file): adapted from upstream `Leonxlnx/taste-skill` v2
  ...
  *one radius*, *motion-must-be-motivated* rules verbatim. Removed the
  ...
  cubecloud-desktop's actual code.
- v1 (previous file): the original `design-taste-frontend` skill
  written for cubecloud-desktop in June 2026, before upstream's v2
  rewrite. Replaced.
