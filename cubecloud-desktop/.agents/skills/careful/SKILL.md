...
- `rm -rf` inside the cubecloud-desktop `__pycache__` or `*.pyc` files
...
In cubecloud-desktop, `careful` can be wired into the main process's IPC handler. The renderer sends a `careful:check` event with the command string; the main process returns `safe | warn | block`:
