...
// instead of cubecloud-desktop's, and bundled the wrong tree. The
...
// enabled in CI — the slow part is the build, not the verification.
//
...
// Resolve the asar path. The test runs from cubecloud-desktop/,
...
describe.skipIf(skipReason !== null)("release bundle integrity (V2.10.43a regression)", () => {
  ...
  it("asar has the inner app's package.json (not the monorepo root's)", () => {
    ...
    // root's package.json does NOT have this field — it has a workspaces
  ...
  it("out/renderer/index.html is real HTML (not a stray node_modules file)", () => {
    ...
    // Spot-check that there's a CSP — the real index.html has one.
    expect(html.toLowerCase()).toContain("content-security-policy");
  });

  it("asar is reasonably sized (under 2 GB — sanity bound)", () => {
  ...
  it("asar top-level is bounded (under 50 entries — sanity bound)", () => {
