...
describe("generateWingetManifests", () => {
  it("produces three YAML files under the winget-pkgs directory layout", () => {
    ...
    expect(
      existsSync(join(outDir, "Cubecloud.CubecloudDesktop.installer.yaml")),
    ).toBe(true);
    expect(
      existsSync(join(outDir, "Cubecloud.CubecloudDesktop.locale.en-US.yaml")),
    ).toBe(true);
    expect(existsSync(join(outDir, "Cubecloud.CubecloudDesktop.yaml"))).toBe(
      true,
    );
  });
  ...
  it("replaces all placeholders in the installer manifest", () => {
    ...
    const outFile = join(
      ...
      "CubecloudDesktop",
      "9.9.9",
      "Cubecloud.CubecloudDesktop.installer.yaml",
    );
