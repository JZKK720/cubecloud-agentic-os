...
describe("OpenClaw WSL handoff helpers", () => {
  it("builds a WSL install script that installs and onboards OpenClaw", async () => {
    const { buildOpenClawWslInstallScript } = await import(
      "../src/main/installer"
    );

    const script = buildOpenClawWslInstallScript();

    expect(script).toContain("npm install -g openclaw@latest");
    expect(script).toContain("openclaw onboard --install-daemon");
    expect(script).toContain("Node.js 22.19+");
    expect(script).toContain("http://127.0.0.1:18789/v1");
  });

  it("builds cmd.exe launch args that open a WSL onboarding shell", async () => {
    const { buildOpenClawWslLaunchArgs } = await import(
      "../src/main/installer"
    );

    expect(buildOpenClawWslLaunchArgs("echo hi")).toEqual([
      "/c",
      "start",
      "",
      "wsl.exe",
