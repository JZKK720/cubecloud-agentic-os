...
export const PROVIDER_BASE_URLS: Record<string, string> = {
  ...
  // Local-LLM providers — OpenAI-compatible shim surfaces that ship
  // with the most common self-hosted runtimes. `ollama serve` and
  // LM Studio both expose `/v1/models`; vLLM and llama.cpp ship
  // the same shape when started with their OpenAI server flag.
  // These defaults are used when the user adds a model via the
  // Providers / Models screens without overriding the Base URL —
  // otherwise the Models page has to ask the user to type
  // 127.0.0.1:11434 by hand on every add.
