 ...
 *   any UI list — ECC, gbrian, gstack, andrej-karpathy-skills,
...
export interface HiddenSkill {
  ...
  readonly id: string;
  /** Short human label — only used internally (logs, debugging). */
   ...
   * this are truncated with a `…` marker. Defaults to 4000 — enough
 ...
 *     against `process.cwd()` (which is the `cubecloud-desktop/`
...
const HIDDEN_SKILLS: readonly HiddenSkill[] = [
  {
    id: "ecc",
    label: "ECC harness — error-correction & continuity",
    intentTags: [
      "ecc",
      "error correction",
      "harness",
  ...
  {
    id: "gbrian",
    label: "gbrian — global brand & research aid",
    intentTags: [
      "gbrian",
      "brand",
      "research",
  ...
  {
    id: "gstack",
    label: "gstack — Godot / game-stack helper",
    intentTags: [
      "gstack",
      "godot",
      "gdscript",
  ...
  },
  {
    ...
    // The Karpathy skill ships a public GitHub source — the desktop
  ...
  {
    id: "taste-skill",
    label: "Frontend taste — design heuristics for shipping UI",
    intentTags: [
      "taste",
      "frontend",
      "ui",
  ...
  {
    // V2.9 pre-launch bundle — operator tone flavor.
    // Injected on every message; body is empty so the prompt stays
    // small. The flavor is the *label* only.
    id: "cubecloud-tone",
    label: "Cubecloud operator tone — concise, action-shaped, honest about limits",
    intentTags: [],
    includeBody: false,
  },
  {
    // V2.9 pre-launch bundle — cost-aware flavor.
    // Injects when the user mentions cost, budget, or model selection.
    id: "cubecloud-economist",
    label: "Cost-aware model and tool selection — pick the smallest model that can do the job",
    intentTags: [
      "cost",
      "budget",
      "expensive",
  ...
  {
    // V2.9 pre-launch bundle — license-aware flavor.
    // Injects when the user mentions license, commercial use, or
    // distribution — references the Cubecloud dual-license posture.
    id: "cubecloud-licensor",
    label: "License-aware — Cubecloud-original is dual-licensed; respect the inherited-MIT framework carve-out",
    intentTags: [
      "license",
      "licence",
      "commercial",
 ...
 * when the file is unreadable. Hard-fails silently — the harness
...
function readHiddenSkillBody(skill: HiddenSkill): string {
  ...
  if (skill.skillPath) {
    ...
    // (cubecloud-desktop/ in dev, app dir in packaged build).
  ...
  // No file found — the skill either has no SKILL.md (e.g. ECC is
 ...
 * the system prompt stays focused — three concurrent hidden
