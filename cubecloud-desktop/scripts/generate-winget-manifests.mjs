...
export function generateWingetManifests({
  ...
  const installerUrl = `https://github.com/${publishOwner}/${publishRepo}/releases/download/v${version}/${name}-${version}-setup.exe`;
  const releaseNotesUrl = `https://github.com/${publishOwner}/${publishRepo}/releases/tag/v${version}`;
  ...
  const files = [
    ["Installer.template.yaml", "Cubecloud.CubecloudDesktop.installer.yaml"],
    [
      "Locale.en-US.template.yaml",
      "Cubecloud.CubecloudDesktop.locale.en-US.yaml",
    ],
    ["Version.template.yaml", "Cubecloud.CubecloudDesktop.yaml"],
  ];
