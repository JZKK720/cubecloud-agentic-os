...
| 成果 | Cubecloudの実現方法 |
|---|---|
| 最初の価値あるセッションまでの時間短縮 | プレローンチバンドルは、3 つのユーザー向けスキル（`cubecloud-persona`、`cubecloud-onboarding`、`cubegraph-code-intel`）、5 つのサンプルタスクを含むスターターかんばんボード、デフォルトモデルレジストリをデスクトップに投入します。ツール、エージェント、スキル、スケジュール、メモリ、ペルソナ、ナレッジグラフ、ファイル管理の各画面は初日からすべて有効で、ユーザーは空のシェルを見つめるのではなく、すぐ に操作を開始できます。 |
...
- [`.agents/skills/`](.agents/skills/) には {{SKILLS_REPOS}} 個のア ップストリームリポジトリから適応された 49 個のオープンソーススキルが含まれます。これらは**コントリビューター向け**のスキルであり、本 monorepo 内で Copilot / Claude Code を動かすためのソースツリーに存在します。**デスクトップバイナリには同梱されません**。デスクトップのエンドユーザーが見るのは別の一 組です：[asar 内に同梱される 28 個のデスクトップ組み込みスキル](agent-desktop/.agents/skills/) で、Skills → Browse タブに表示されます。3 層すべての内訳は [`agent-desktop/README.md`](agent-desktop/README.md#skills-ecosystem--3-layers) を参照してください。
...
- **3 つのユーザー向けスキル**（`cubecloud-persona`、`cubecloud-onboarding`、`cubegraph-code-intel`）、**5 つのサンプルタスクを含むスターターかんばんボード**、エージェントが初日から話せるプロバイダを一覧する**デフォルトモデルレジストリ**を含むプレローンチ運用コンテキスト。Tools、Agents、Skills、Schedules、Memory、Soul (ペルソナ)、Memory (wiki)、Workspace (ファイル)、Settings の各サーフェスは初日からすべて稼働しており、すぐに使えます。
...
├── .agents/                      ~/.agents/skills/ にミラーリングされる{{SKILLS_TOTAL}}のオープンソーススキル
