...
The V2.3 (Steps 1闂?) rollout shipped alongside the following
branding and license hygiene work:

- **`LICENSE` attribution fixed.** The prior `Copyright (c) 2026
  github.com/fathah` line was incorrect 闂?`fathah` is a personal
...
- **Legacy brand assets removed.** `src/renderer/src/assets/`
  ...
  `splash.png`, `splashtext.png`, and `splashtext-w.webp` 闂?
...
- Cubecloud Agent Desktop is built on the `hermes-desktop`
  ...
  on top of that framework 闂?the renderer rebuilds, the state
  layer, the V2.3 modules, the SQLite schema, the
  provider-discovery logic, the hidden skills harness, the
  smoke / capture scripts, and the architecture docs 闂?is
...
- **`NOTICE` 闂?Adapted dependencies" rewritten.** The
  ...
  tree.
- **`ACKNOWLEDGMENTS.md` 闂?Skills adapted from
...
reference* 闂?it is the original Karpathy harness preserved
...
top-level LICENSE is a dual-license notice 闂?AGPL-3.0-or-later
...
- **14 new `sp-` skills added to `.agents/skills/`.** One
  ...
  - `sp-skill-first` (from `using-superpowers`) 闂?the
    bootstrap: before any response, check for skills.
  - `sp-tdd` (from `test-driven-development`) 闂?
    RED-GREEN-REFACTOR with anti-patterns.
  - `sp-debug` (from `systematic-debugging`) 闂?4-phase
    root-cause process. Heavier than `po-diagnose`.
  - `sp-verify` (from `verification-before-completion`) 闂?
    evidence over claims. The 5 verification questions.
  - `sp-brainstorm` (from `brainstorming`) 闂?Socratic
    design refinement. Activates before any implementation.
  - `sp-plan` (from `writing-plans`) 闂?bite-sized tasks
    (2闂? min) with exact file paths, complete code,
    verification steps.
  - `sp-execute` (from `executing-plans`) 闂?run the plan
    task-by-task with 2-stage review.
  - `sp-subagents` (from `subagent-driven-development`) 闂?
    parallel subagent dispatch with 2-stage review.
  - `sp-parallel` (from `dispatching-parallel-agents`) 闂?
    one-off parallel queries for research.
  - `sp-request-review` (from `requesting-code-review`) 闂?
    pre-review checklist; severity-ordered findings.
  - `sp-receive-review` (from `receiving-code-review`) 闂?
    triage, fix, defend, push back.
  - `sp-worktree` (from `using-git-worktrees`) 闂?isolated
    worktree, clean baseline, implement, verify.
  - `sp-finish-branch` (from
    `finishing-a-development-branch`) 闂?verify, present
    4 options, clean up.
  - `sp-write-skill` (from `writing-skills`) 闂?
...
- **1 new `licenses/<repo>-MIT.txt` file vendored.**
  `licenses/superpowers-MIT.txt` 闂?the upstream MIT text
...
- **`.agents/skills/README.md` updated to a 34-skill
  ...
  (`sp-brainstorm` 闂?`sp-plan` 闂?`sp-execute` /
  `sp-subagents` 闂?`sp-finish-branch`), and updates the
...
- **`NOTICE` 闂?Adapted dependencies" 闂?Skills ecosystem"
...
- **`ACKNOWLEDGMENTS.md` 闂?Skills adapted from third-party
...
### The V2.8 description-trim audit (DONE 闂?see 闂?V2.8 transitions landed" above)
...
`sp-write-skill` 闂?The Description Trap".
...
V2.8 diff is in the 闂?V2.8 transitions landed" section above.
...
a few adjacent open-source projects during design 闂?most
notably [Odysseus](https://github.com/pewdiepie-archdaemon/odysseus)
闂?and **borrowed their rebrand and licensing posture as a model**
...
- **`LICENSE` copyright line is now `Copyright (c) 2026 Cubecloud
  ...
  saw on the Odysseus project 闂?a contributors collective,
...
framework code is, and remains, hard-MIT 闂?we cannot
...
- **`docs/legal/COMMERCIAL_LICENSE.md` added.** This new
  ...
  service without the AGPL-3.0 闂?3 network-source
...
deployment and AGPL-3.0 闂?3 does not trigger. The AGPL-3.0
clause that matters is 闂?3 闂?the "remote network interaction"
...
"free alternatives" to AGPL-3.0 闂?they are alternative
...
- **20 V2.6 skill descriptions trimmed.** Each description
  ...
  starts with Use when闂?(or Use at the end闂?for
...
ame: field was
  ...
  description started with Use after闂?instead of
  Use when闂?. Both classes of defect are fixed. The
...
Per sp-write-skill 闂?The Description Trap" (and per upstream
...
- **5 new Cubecloud-original files in
  ...
  - defaultSkills.ts 闂?3 user-visible skills
    (cubecloud-persona, cubecloud-onboarding,
    cubegraph-code-intel), all in the Skills tab.
  - defaultMemories.ts 闂?6 memory seeds covering
    conventions, runtime topology, two-tier skills, license
    / brand, workspace conventions, security posture.
  - defaultHarnesses.ts 闂?3 disabled EverOS harnesses
    (cubecloud-memory-distill, cubecloud-cost-watchdog,
    cubecloud-skill-audit).
  - defaultSchedules.ts 闂?1 disabled schedule
    (cubecloud-daily-standup).
  - defaultKanban.ts 闂?1 starter board ("Onboarding 闂?    delete me") with 5 deletable example tasks.
...
- **3 new hidden-flavor skills in
  ...
  - cubecloud-tone (intentTags: empty) 闂?operator-tone
    flavor that auto-injects on every message.
  - cubecloud-economist (intentTags: cost, budget,
    model selection) 闂?cost-aware model/tool selection.
  - cubecloud-licensor (intentTags: license,
    commercial, distribution) 闂?license-aware responses.
...
- **5 new entries in NOTICE 闂?Direct dependencies 闂?  Cubecloud-original work (2026)"**. The 5 new files in
...
re-add the defaults. The Settings 闂?Advanced 闂?"Reset to defaults" button (V2.10) will use this
behaviour to re-seed.

A future V2.10 will add a settings toggle to disable
the seed entirely (Settings 闂?Advanced 闂?"Pre-launch
