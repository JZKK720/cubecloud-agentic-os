...
- **Works offline.**
- **No cloud API fees.**
- **OEM ready.**  *(The fourth item, present on the CTA strip but not in the user's three-line summary. Kept here for completeness — it signals that the platform ships as an OEM appliance, not just a downloadable installer.)*

The corporate identity, drawn from the cubecloud.io footer (every page):  `[SRC: footer]`

- **Legal entity**: Cubecloud Limited Company / 智方云
- **Copyright**: © 2026
- **License**: AGPL-3.0-only
- **Trademarks**: Cubecloud, 智方云, and cubecloud.io branding and trademarks reserved
- **Bilingual brand mark**: SOVEREIGN INTELLIGENCE / 主权智能化

### The product surface: Cubecloud OS

The cubecloud.io homepage introduces the product as **Cubecloud OS** — *the private AI operating layer*.  `[SRC: homepage graph caption]`

The cubecloud.io manifest page is the canonical statement of what Cubecloud OS *is*:  `[SRC: manifest page]`

> "The platform operating layer is open-source and ever-growing. Surfaces can be added, removed, or swapped freely as team needs change — there is no fixed set of tools."
...
- **Agents** — long-lived processes with a defined role.
- **Skills** — workflow bundles agents can load.
- **Data** — per-user state, persisted locally.
- **Local PC Hardware** — the substrate.
...
The hardware page also notes that the four SKUs are sample configurations matched to "demos, office nodes, and higher-throughput teams" — i.e., they are not the entire catalog.  `[SRC: hardware page]`
...
- **Phase 1 — Services**: deployment and professional services.
- **Phase 2 — SaaS**: managed support and annual subscriptions.
- **Phase 3 — OEM**: OEM licensing once delivery patterns are proven.
...
> **Footnote — internal brand framing**  `[SRC: user, 2026-06-08]`
>
...
- **The agent runtime** — the headless agent control plane in `cubecloud-desktop/src/main/` and the renderer surface in `cubecloud-desktop/src/renderer/`. Each agent is a long-lived process with a defined role, a memory store, and a skills bundle.
- **The skills bundles** — the `.agents/skills/**/SKILL.md` packs and the in-app skills registry that lets an agent load a workflow bundle on demand. The same packs are mirrored at the desktop layer (`cubecloud-desktop/.agents/skills/`) for the in-app agent runtime.
- **The per-user data layer** — memory, kanban, schedules, skills registry, and Headroom config (when used), persisted locally in the user's own `desktop.json` (paths resolved at runtime by the desktop shell). No telemetry, no remote sync, no account system.
- **The desktop shell** — the Electron application that runs on the operator's own machine.
- **The i18n layer** — 8 locales (en, es, id, ja, pt-BR, pt-PT, zh-CN, zh-TW), with en + zh-CN as the source-of-truth locales and the others falling back to en.
- **The Headroom workflow layer** — a developer aid for token-context compression; not a runtime dependency of the agent.
- **The docs** — the V2.10.x history, the brand-license provenance, the inner / outer doc mirror layer, and the manifest of what's live vs. scratch vs. mirror.
...
| **Open WebUI** *(private AI workspace)* | The chat / retrieval / tool-use surface in `cubecloud-desktop/src/renderer/src/`. Integrations with Ollama, llama.cpp, LM Studio.  `[SRC: demos page]` |
| **Hermes** *(memory, schedule, subagents)* | `cubecloud-desktop/src/main/hermes*` and the in-app memory / schedule / subagent registry.  `[SRC: demos page, with repo audit]` |
...
| **Local PC Hardware** substrate | The Electron app's runtime model — the desktop shell runs on the operator's own machine, the agent control plane runs as a long-lived process on the same machine, no remote server is required. The four supported hardware SKUs (Mini AIPC, NEX-AI + iGPU bundle, ColorFire MEOW AI laptop, ColorFire AI MODT workstation) are listed in the [hardware lineup](#the-hardware-lineup) section above.  `[SRC: hardware page]` |
...
- **Trademarks**: Cubecloud, 智方云, and cubecloud.io are reserved trademarks of Cubecloud Limited Company. Use of these marks in forks, derivatives, or downstream distributions is governed by `BRANDING_AND_LICENSE.md` and the cubecloud.io footer.  `[SRC: footer]`
...
- **Commercial context**: The repository is part of the cubecloud.io product line, which is structured per the materials page as Phase 1 (services) → Phase 2 (SaaS) → Phase 3 (OEM). The repository itself is the source-available view of the operating layer; the commercial phasing is for the cubecloud.io business, not for this repo's contribution cadence.  `[SRC: materials page]`
...
1. `README.md` — the marketplace framing, the 8-locale support, the conceptual model (object + action).
2. `BRANDING_AND_LICENSE.md` — the V2.x history and the brand-license provenance. Required reading if you intend to fork, rebrand, or distribute.
3. `docs/HANDBOOK.md` — the master handbook; architecture, development, operations.
4. `docs/RETIRED_AND_LEGACY.md` — what is live, what is scratch, what is mirror.
5. `cubecloud-desktop/` — the desktop shell, with its own `README.md`, `AGENTS.md`, and `docs/` mirror.
6. `.agents/skills/` and the desktop mirror at `cubecloud-desktop/.agents/skills/` — the skill packs available to the in-app agent.
