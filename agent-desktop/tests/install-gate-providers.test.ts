...
describe("expectedEnvKeyForModel — provider-name lookup", () => {
  it.each([
    ...
    // Local-LLM providers — same shape as hosted ones, just with
    // optional keys. The user can run Ollama / LM Studio with no
    // auth, but if they turn on a server-side key we surface it.
...
describe("expectedEnvKeyForModel — URL fallback for custom/auto providers", () => {
  ...
  it("maps Ollama / LM Studio loopback URLs to their named env keys", () => {
    // The base-URL fallback wins for users with `custom` /
    // `auto` providers pointed at a local Ollama or LM Studio
    // install. Named rules match before the generic `_API_KEY`
    // permissive scan, so the install-gate surfaces a useful
    // hint in the prompt ("set OLLAMA_API_KEY or leave it empty")
    // rather than leaving the user guessing.
    expect(
      expectedEnvKeyForModel("custom", "http://127.0.0.1:11434/v1"),
    ).toBe("OLLAMA_API_KEY");
    expect(
      expectedEnvKeyForModel("custom", "http://localhost:11434/v1"),
    ).toBe("OLLAMA_API_KEY");
    expect(
      expectedEnvKeyForModel("custom", "http://127.0.0.1:1234/v1"),
    ).toBe("LMSTUDIO_API_KEY");
    expect(
