...
vi.mock("../src/main/config", () => ({
  ...
  getConnectionConfig: () => ({
    ...
    apiKey: "",
    gatewayRuntimePreset: "hermes" as const,
    ssh: {
      host: "",
      port: 22,
      username: "",
...
describe("normaliseRemoteUrl", () => {
  ...
  it("maps ws and wss URLs onto the matching HTTP scheme", () => {
    expect(normaliseRemoteUrl("ws://127.0.0.1:18789/v1")).toBe(
      "http://127.0.0.1:18789",
    );
    expect(normaliseRemoteUrl("wss://gateway.example.com/v1")).toBe(
...
describe("testRemoteConnection URL probe", () => {
  ...
  it("recognizes an OpenClaw gateway through /v1/models when /health is absent", async () => {
    const capturedTargets: string[] = [];
    const reqSpy = vi
      .spyOn(http, "request")
      .mockImplementation((target: unknown, ...rest: unknown[]) => {
        const targetUrl = String(target);
        capturedTargets.push(targetUrl);
        const cb = rest[rest.length - 1] as (res: unknown) => void;
        ...
        if (targetUrl.endsWith("/health")) {
          ...
          queueMicrotask(() => {
            for (const handler of listeners.data ?? []) {
              handler(Buffer.from('{"data":[{"id":"openclaw/default"}]}'));
            }
  ...
  it("surfaces an OpenClaw compatibility diagnostic when /v1/models never appears", async () => {
    const reqSpy = vi
      .spyOn(http, "request")
      .mockImplementation((target: unknown, ...rest: unknown[]) => {
        const cb = rest[rest.length - 1] as (res: unknown) => void;
    ...
    try {
      await expect(
        diagnoseRemoteConnection("http://127.0.0.1:18789/v1", "openclaw"),
