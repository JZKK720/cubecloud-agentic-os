...
describe("provider-registry", () => {
  describe("canonicalProviderBaseUrl", () => {
    ...
    it("returns the canonical URL for the named local-LLM providers", () => {
      // Ollama + LM Studio defaults land on the loopback OpenAI-compat
      // shim port. The /v1 suffix is what the OpenAI shim serves;
      // the model-discovery probe strips it to reach Ollama's native
      // /api/tags endpoint when the shim returns empty.
      ...
      // Case-insensitive lookup — `discoverProviderModels` always
      // lower-cases the provider before consulting the table.
      expect(canonicalProviderBaseUrl("OLLAMA")).toBe(
        "http://127.0.0.1:11434/v1",
      );
      expect(canonicalProviderBaseUrl("LmStudio")).toBe(
      ...
      // Sanity: the raw map exposes the same entries.
      expect(PROVIDER_BASE_URLS.ollama).toBe("http://127.0.0.1:11434/v1");
      expect(PROVIDER_BASE_URLS.lmstudio).toBe("http://127.0.0.1:1234/v1");
    });
