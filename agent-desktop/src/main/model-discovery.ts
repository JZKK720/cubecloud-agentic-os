...
function parseModelIds(body: string): string[] {
  ...
  // Ollama native shape: { models: [{ name: "llama3.2:latest" }, ...] }.
  // We hit `/api/tags` when the OpenAI-compat `/v1/models` returns
  // empty — Ollama still ships that endpoint natively on every
  // install even when the OpenAI shim is disabled. The `name`
  // already includes the tag (e.g. `llama3.2:latest`), so the
  // string round-trips through the model field as-is. Same shape
  // also covers Anthropic-style `models: [{ id, name }]` for any
  // provider that adopts the alternative envelope.
...
/** Try Ollama's native `/api/tags` catalogue. Returns the model names
 *  (which already include the tag — `llama3.2:latest`) or [] on any
 *  failure. Used as a fallback when the OpenAI-compat `/v1/models`
 *  endpoint is disabled on the user's Ollama install (older versions,
 *  or `OLLAMA_ORIGINS` whitelisting off). The native endpoint is
 *  always available, so this is the most reliable Ollama probe.
 *
 *  We don't reuse `fetchModelsHttp` because the response shape is
 *  different — `parseModelIds` handles both, but the upstream
 *  `Authorization` header isn't needed for an unauthenticated
 *  loopback Ollama. Sending `Authorization: Bearer ""` is harmless
 *  but adds noise to Ollama's log. */
async function fetchOllamaTagsHttp(
  baseUrl: string,
  timeoutMs: number,
): Promise<string[]> {
  const trimmed = baseUrl.replace(/\/+$/, "");
  // `/v1` suffix is the OpenAI compat shim; the native API sits at
  // the host root. Strip `/v1` (and any path) to land on the native
  // endpoint.
  const nativeBase = trimmed.replace(/\/v1(?:\/.*)?$/, "");
  const url = `${nativeBase}/api/tags`;
  return new Promise((resolve) => {
    const u = new URL(url);
    const mod = u.protocol === "https:" ? https : http;
    const req = mod.request(
...
export async function discoverProviderModels(
  ...
  // Local-LLM providers (Ollama, LM Studio) don't require an API key
  // for `/v1/models` — they accept the call anonymously. We still
  // honour a user-supplied `OLLAMA_API_KEY` / `LMSTUDIO_API_KEY`
  // if it's set, so a server with auth on a non-default port keeps
  // working. The "no key" early-return below only fires for hosted
  // providers that genuinely need one.
  const isLocalLlm = isLocalLlmProvider(lowerProvider);
  const apiKey =
    (apiKeyOverride || "").trim() ||
  ...
  const headers = isLocalLlm && !apiKey
    ? ({} as Record<string, string>)
    : authHeaders(lowerProvider, apiKey);
  let models = await fetchModelsHttp(url, headers, 10_000);
  // Ollama fallback: if the OpenAI-compat `/v1/models` returns no
  // models (older Ollama builds, or a deployment that disabled
  // the shim), hit the native `/api/tags` catalogue. Same parse
  // path — `parseModelIds` understands both shapes.
  if (models.length === 0 && lowerProvider === "ollama") {
    const native = await fetchOllamaTagsHttp(baseUrl, 10_000);
    if (native.length > 0) models = native;
  }
  setCache(lowerProvider, baseUrl, models);
  return { models, status: "ok", cached: false };
}

/** True when `provider` is a self-hosted OpenAI-compatible shim that
 *  accepts anonymous catalogue requests (Ollama, LM Studio, vLLM,
 *  llama.cpp). Used to skip the `*_API_KEY` requirement that hosted
 *  providers need — a user who hasn't set `OLLAMA_API_KEY` should
 *  still be able to browse whatever their local Ollama instance
 *  has pulled. */
function isLocalLlmProvider(provider: string): boolean {
  const p = provider.trim().toLowerCase();
  if (p === "ollama" || p === "lmstudio") return true;
  // Custom OpenAI-compatible endpoints on a private/loopback host
  // also fall in this bucket: a user adding
  // `http://192.168.1.50:8000/v1` for their home-server vLLM clearly
  // doesn't need a key in .env to discover models.
  return false;
}
