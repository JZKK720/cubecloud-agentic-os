...
export function getConnectionConfig(): ConnectionConfig {
  ...
  const gatewayRuntimePreset = resolveGatewayRuntimePreset({
    storedPreset: data.gatewayRuntimePreset,
    remoteUrl,
    sshRemotePort: ssh.remotePort as number,
  });
  return {
    ...
    ssh: {
      ...
      remotePort: (ssh.remotePort as number) || DEFAULT_SSH_REMOTE_PORT,
      localPort: (ssh.localPort as number) || DEFAULT_SSH_LOCAL_PORT,
    },
  };
}
...
export function getPublicConnectionConfig(): PublicConnectionConfig {
  ...
  return {
    ...
    gatewayRuntimePreset: config.gatewayRuntimePreset,
    ssh: config.ssh,
  };
}
...
export function setConnectionConfig(config: ConnectionConfig): void {
  ...
  data.gatewayRuntimePreset = config.gatewayRuntimePreset;
  if (config.mode === "ssh") {
...
export function resolveConnectionGatewayRuntimePresetUpdate(
  existing: ConnectionConfig,
  args: {
    mode: "local" | "remote" | "ssh";
    remoteUrl?: string;
  ...
  if (args.gatewayRuntimePreset) {
    return args.gatewayRuntimePreset;
  }

  if (args.mode === "remote") {
    return inferGatewayRuntimePreset({ remoteUrl: args.remoteUrl });
  }

  if (args.mode === "ssh") {
    return inferGatewayRuntimePreset({ sshRemotePort: args.sshRemotePort });
  }

  return existing.gatewayRuntimePreset;
}
 ...
 * (NousResearch/hermes-agent #?? — see legacy desktop issue #260): the
