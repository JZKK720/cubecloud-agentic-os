...
function Skills({ profile }: SkillsProps): React.JSX.Element {
  ...
  // Open the raw SKILL.md for desktop-bundled skills. We resolve the path
  // via listInstalledSkills()'s shape: a flat `<root>/<skill>/SKILL.md`.
  // The desktop bundled source ships under .agents/skills/, so we ask the
          ...
          {filteredBundled.map((skill) => {
            ...
            return (
              <div
                ...
                className="skills-card"
              >
                <div className="skills-card-category-row">
                  <span className="skills-card-category">{skill.category}</span>
                  <span
                    className={`skills-card-source skills-card-source--${
                      isDesktop ? "desktop" : "hermes"
                    }`}
                    title={sourceLabel(skill.source)}
                  >
                ...
                <div className="skills-card-footer">
                  {isDesktop ? (
                    <button
                      className="btn btn-secondary btn-sm"
                      onClick={() => handleViewBundled(skill)}
                    >
                      {t("skills.view")}
                    </button>
                  ) : isInstalled ? (
