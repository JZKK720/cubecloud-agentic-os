...
export function detectProviderFromUrl(rawUrl: string): string | null {
  ...
  if (/api\.siliconflow\.com/.test(url)) return "siliconflow";
  if (/api\.novita\.ai/.test(url)) return "novita";
  ...
  // Atomic Chat 1337. We use a *named* provider id (not "custom") so
  // the model card displays "Ollama" / "LM Studio" rather than the
  // generic "OpenAI Compatible / Local" label. Loopback is not
  // required for the port-only heuristic — a user running Ollama
  // on a LAN host still benefits from the named card label.
