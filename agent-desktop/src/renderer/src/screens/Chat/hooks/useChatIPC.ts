  ...
  useEffect(() => {
    ...
    const cleanupUsage = window.hermesAPI.onChatUsage((u) => {
      setUsage((prev) => ({
        ...
        // Headroom stats are per-turn: the main process
        // emits them on the final usage chunk. We
        // overwrite (not accumulate) so the footer shows
        // the savings for the most recent turn, not a sum
        // across the whole session. When the field is
        // absent (no compression attempted), we keep the
        // previous turn's value so a chat with a mix of
        // compressed and uncompressed turns still has a
        // visible history of the last compression.
        headroom:
          u.headroom !== undefined
            ? u.headroom
            : prev?.headroom,
      }));
    });
