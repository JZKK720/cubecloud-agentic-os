...
> (`2.10.68` → `2.10.71`) is the honest version to ship. `v0.6.x` would
...
**Published on GitHub Release
[v2.10.71](https://github.com/JZKK720/cubecloud-agentic-os/releases/tag/v2.10.71):**

| Artifact | Size | SHA-256 | Published? |
|---|---|---|---|
| `cubecloud-agent-desktop-2.10.71-setup.exe` | 130,448,269 bytes | `A1E83A61EFA3DB14D96BD341EDA7D319DDB8837424CA7AF66231E589637755C0` | ✅ uploaded |
| `cubecloud-agent-desktop-2.10.71-setup.exe.blockmap` | 136,220 bytes | `D7D8262B8FAD08952649AC6BB4365EC8A04D408DC40B957F7AC221FFBFD56330` | ✅ uploaded |
| `latest.yml` | 381 bytes | `EC9A43081CA874ED9A0101F7A612213E3F67A0AA73988312FFEE51EF54C99A2D` | ✅ uploaded |

**Built but NOT uploaded (corporate proxy blocked the 130 MB transfer):**

| Artifact | Size | SHA-256 | Published? |
|---|---|---|---|
| `cubecloud-agent-desktop-2.10.71-portable.exe` | 130,289,648 bytes | `69DDDB681905913F155165836618C70DED3AA16F97D785C4E8A8A2BAE99CDC4B` | ❌ not uploaded |

The portable.exe was built correctly and its SHA-256 is recorded
above for the artifact on the build host, but it was never
delivered to the GitHub release. End users on Windows install
via `setup.exe` (the NSIS wizard path), which is the primary
install path.
...
- [x] **Diagnosed the asar regression.** Root cause: V2.10.43c added
...
- [x] **Fix applied:** un-list `agent-desktop` from npm workspaces so it
  installs its own deps locally (660 entries), exactly like v0.6.1 did.
  `agent-desktop/` is now a standalone buildable project.
- [x] **Rebuild verified:** asar 176.92 MB, 21,291 `node_modules/`
  entries, 214 `out/` entries. `verify:bundle` — all 7 tests PASS.
- [x] **Version bumped:** `agent-desktop/package.json` 2.10.68 → 2.10.71.
  Root `package.json` stays at 2.10.68.
- [x] **Installers built:** `setup.exe` (130 MB), `portable.exe`
  (130 MB), `.blockmap`, `latest.yml`. SHA-256s recorded above.
- [x] **Tag `v2.10.71`** at the verified commit, push the tag.
- [x] **`gh release create v2.10.71`** with this file as `--notes-file`
  and the four build artifacts attached.
- [x] **`gh release edit v0.6.1 --prerelease`** to retire the wrapper-era
  release.
