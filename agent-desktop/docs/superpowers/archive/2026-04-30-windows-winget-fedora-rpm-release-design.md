...
- Automated PR submission to `microsoft/winget-pkgs` (would require a GitHub PAT in repository secrets, which the PR author does not control).
...
â”œâ”€ prepare              [ubuntu]   unchanged + new is_dry_run output
â”œâ”€ release_mac          [macos]    unchanged
â”œâ”€ release_linux        [ubuntu]   ADD: rpm to electron-builder targets, install rpm apt package
â”œâ”€ release_windows      [windows]  NEW: NSIS .exe + .blockmap + latest.yml
â”œâ”€ generate_winget      [ubuntu]   NEW: SHA256 the .exe, fill manifest templates, upload as artifact
â””â”€ publish              [ubuntu]   gated: skip if is_dry_run == 'true'
```
...
- Add new job `generate_winget`:
  ...
  - `if: needs.prepare.outputs.tag_exists == 'false'`
  - Steps: checkout, download `windows-artifacts` to `dist/`, run `node scripts/generate-winget-manifests.mjs` with env `VERSION` and `PUBLISH_OWNER=cubecloud`, upload `dist/winget/` as artifact `winget-manifests-${{ needs.prepare.outputs.version }}`.
- Modify `publish`:
...
- Final value of `linux.vendor` (`Cubecloud` vs `Nous Research`).
...
- Auto-submit winget PR via `vedantmgoyal2009/winget-releaser` action (requires GitHub PAT in repository secrets).
