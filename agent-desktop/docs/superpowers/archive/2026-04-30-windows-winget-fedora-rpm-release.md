...
//
// Run from CLI: VERSION=0.2.3 PUBLISH_OWNER=cubecloud node scripts/generate-winget-manifests.mjs
...
if (isCli) {
  ...
  const result = generateWingetManifests({
    ...
    name: pkg.name,
    publishOwner: process.env.PUBLISH_OWNER || "cubecloud",
  });
...
- `publishOwner: "cubecloud"` — same in workflow env (Task 15) and CLI default (Task 9).
