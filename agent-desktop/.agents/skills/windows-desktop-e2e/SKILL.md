...
Em-dashes (`â€”`) typed into a prompt can survive a `create_file` round-trip as two literal `?` characters (`??`). The file is then saved with mojibake. Always `Format-Hex` lines that are supposed to contain em-dashes:
