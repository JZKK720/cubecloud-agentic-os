...
`scripts/smoke-all.js` orchestrates the existing CDP-based `verify-*`
and visual `capture-*` scripts in sequence, giving a single
"is the build healthy?" command. Use it before a release or after
landing a large PR.

```bash
ENABLE_CDP=1 npm run dev          # terminal 1
node scripts/preview-mock-gateway.js  # terminal 2 (optional)
npm run smoke                     # terminal 3
```

The current suite (in order, all run sequentially against the
single CDP-attached browser context):

| Script | Purpose | Skipped by default? |
|--------|---------|---------------------|
| `verify-step3-4-ipc` | CodeGraph runtime + EverOS sidecar IPC channels (12 channels) | – |
| `verify-everything` | Broader IPC surface — 80 read-only + in-staging channels across locale, profile, kanban, plans, knowledge, learnings, retro/triage/handoff, careful, discovery, files | – |
| `verify-nous-discovery` | Nous provider model discovery (network-dependent) | ✅ yes — set `INCLUDE_NETWORK_SUITE=1` to enable |
| `capture-codegraph-everos` | Visual capture of the two new screens (CodeGraph, EverOS) — writes `previews/codegraph.png` + `previews/everos.png` | – |

Each script prints its own `[VERDICT]` lines; the runner aggregates
into a single summary table. The runner exits 1 if any suite returns
non-zero. A suite is **red** if any channel probe returns a real
`TypeError` (typed `Error` envelopes like "Plan not found" are
treated as **green** — the channel is wired up and reports input
problems gracefully).

To add a new suite, append an entry to the `SUITE` array in
`smoke-all.js` with a `script` path and a one-line `description`. The
runner takes care of stdout/stderr capture, per-suite timing, and
