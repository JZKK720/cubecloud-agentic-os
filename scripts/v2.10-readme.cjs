...
// 5. Append a row to docs/RETIRED_AND_LEGACY.md (the "README split" row).
//
// This is a one-shot; delete after V2.10 lands.
...
// Check the LinkType via PowerShell (Node doesn't expose NTFS link type directly)
const { execSync } = require('child_process');
const linkInfo = execSync(`powershell -NoProfile -Command "(Get-Item '${inner}').LinkType"`, { encoding: 'utf8' }).trim();
console.log('  inner LinkType:', linkInfo);

// For the transition, we just overwrite both files. Overwriting a hardlink in
// Node may break the link (Node opens, truncates, writes) -- which is what
// we want here.
console.log('Step 2: write new OUTER README (agentic-OS monorepo)');
fs.writeFileSync(OUTER_README, outerReadme);
console.log('  outer README size:', fs.statSync(OUTER_README).size, 'bytes');

console.log('Step 3: write new INNER README (install + features + providers)');
fs.writeFileSync(inner, innerReadme);
console.log('  inner README size:', fs.statSync(inner).size, 'bytes');

// Verify the hardlink was broken
const outerLink = execSync(`powershell -NoProfile -Command "(Get-Item '${OUTER_README}').LinkType"`, { encoding: 'utf8' }).trim();
const innerLink = execSync(`powershell -NoProfile -Command "(Get-Item '${inner}').LinkType"`, { encoding: 'utf8' }).trim();
console.log('  outer LinkType after write:', outerLink || '(empty = regular file)');
console.log('  inner LinkType after write:', innerLink || '(empty = regular file)');

console.log('Step 4: append V2.10.6 to BRANDING_AND_LICENSE.md');
const brandingSrc = fs.readFileSync(BRANDING, 'utf8');
if (brandingSrc.includes('V2.10.6')) {
  console.log('  V2.10.6 already in BRANDING; skipping');
} else {
  fs.writeFileSync(BRANDING, brandingSrc + brandingV2106);
  console.log('  BRANDING size now:', fs.statSync(BRANDING).size, 'bytes');
}

console.log('Step 5: append README-split row to RETIRED_AND_LEGACY.md');
const retiredSrc = fs.readFileSync(RETIRED, 'utf8');
if (retiredSrc.includes('Outer agentic-OS monorepo README')) {
  console.log('  README-split row already in RETIRED; skipping');
} else {
  // Insert before the "## How to confirm a surface is live" section
  const marker = '## How to confirm a surface is live';
  if (!retiredSrc.includes(marker)) {
    console.error('  marker not found in RETIRED:', marker);
    process.exit(1);
  }
  const newRetired = retiredSrc.replace(marker, retiredAddendum.trimEnd() + '\n\n' + marker);
  fs.writeFileSync(RETIRED, newRetired);
  console.log('  RETIRED size now:', fs.statSync(RETIRED).size, 'bytes');
}
