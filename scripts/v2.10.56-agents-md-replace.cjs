// One-off: replace agent-desktop/AGENTS.md with the v2 content.
// The existing file referenced `electron-pro` and `typescript-expert`
// skills that do not exist in this repo. This version is shorter,
// links (does not embed) the workspace-root AGENTS.md, and only
// carries desktop-specific rules.

const fs = require("fs");
const path = "agent-desktop/AGENTS.md";
const content = `# Agent Instructions — agent-desktop
...
console.log(path + ": wrote " + content.length + " bytes");
