...
#
# scripts/sync-docs.ps1 — Option A: move governance docs to the outer
# cubecloud-agentic-os/ root, replace the inner cubecloud-desktop/ copies
...
$inner = Join-Path $root 'cubecloud-desktop'
...
foreach ($k in $fileMap.Keys) {
    $rel = $k
    if ($rel -match '^docs/') {
        # Some files live in inner docs/ — handle subfolder structure separately
        continue
    }
